...
//backpatch(p, i)：将列表p指向的所有语句中的空白地址用i填入
    ...
    */
    ...
    for(std::vector<BasicBlock*>::iterator it=func->begin();it!=func->end();it++)
    {
        ...
        while(ptr!=(*it)->end())
        {
            ...
            else if(ptr->isUncond())//非条件指令，直接无条件跳转
            {
...
void BinaryExpr::genCode()
{
    ...
    if (op == AND)
    {
        ...
        expr2->genCode();
        //逻辑与具有短路的特性，当第一个子表达式的值为假时，整个布尔表达式的值为假，第二个子表达式不会执行；
        //当第一个子表达式的值为真时，根据第二个子表达式的值得到整个布尔表达式的值。
    ...
    else if(op == OR)
    {
        ...
        unbackPatch(expr1->falseList(), trueBB);//条件为假的跳转地址
...
void Id::genCode()
{
    ...
    if(sp->getScope()==1)
    {
        ...
        if(sp->kput!=0)//第一次遇到（形参），初始化，需要分配临时地址
        {
            //add beg
            SymbolEntry* tmp_se=getSymPtr();//分配符号表项
            ...
            addr_se = new TemporarySymbolEntry(type, SymbolTable::getLabel());
            addr = new Operand(addr_se);//new 一个当前操作数
            Instruction* alloca = new AllocaInstruction(addr, addr_se,nullptr,1);//生成 分配指令 符号表项，地址
            //
            //fprintf(stderr, "gen alloca ins. getlabel:%d\n",SymbolTable::getLabelnotInc()-1);
            //
            entry->insertFront(alloca); //在当前块中插入之前生成的分配指令
    
            addr->get_symbolentry()->setType(new PointerType(type));
            //add beg
            //dst=new Operand(tmp_se);
            //add end
            alloca = new StoreInstruction(addr, dst);//生成 存储指令
            //
            //fprintf(stderr, "gen store ins. getlabel:%d\n",SymbolTable::getLabelnotInc()-1);
            //fprintf(stderr, "dst is %s\n",dst->toStr().c_str());
            //
            entry->insertBack(alloca);// 插入 存储指令
            sp->setAddr(addr);
            sp->kput=0;                
            //testtesttest
    ...
    {
        addr_se = new TemporarySymbolEntry(TypeSystem::intType, SymbolTable::getLabel());//为什么不是IdentifierSymbolEntry
        Operand *fack;
        fack = new Operand(addr_se);
        BasicBlock *bb = builder->getInsertBB();
...
void IfElseStmt::genCode()
{
    ...
    //同样增加前驱后继 实际上都没有用上，不用管
    then_bb->addPred(entry);
    entry->addSucc(then_bb);

    else_bb->addPred(entry);
    entry->addSucc(else_bb);
...
void DeclStmt::genCode()
{
    ...
    if(se->getScope()==0)//全局
    {
        ...
        addr = new Operand(addr_se);
        se->setAddr(addr);//用来存放变量地址信息
        
        //fprintf(stderr, "  break 2\n");
        //分配全局变量地址
        Instruction *alloca;
        //fprintf(stderr, "  break 3\n");
        alloca=new globalAllocaInstruction(addr,se);//记录全局变量地址，并存储值
        Unit *unit = builder->getUnit();
        //fprintf(stderr, "  break 4\n");
        unit->global_push(alloca);
        //fprintf(stderr, "  break 5\n");
    }
    else if(se->getScope()>0)//局部
    {
...
void UnaryExp::genCode()
{
    ...
    if(opcode==UnaryInstruction::NOT)
    {
        ...
        // 如果not后面是一个i32 就要先和0比较 然后对于结果进行取反
        // if (src->getType()->getSize() == 32) 
        // {
    ...
    //     // new BinaryInstruction(opcode, dst, src2, src, bb);
    //     Operand* src2;
    //     BasicBlock* bb = builder->getInsertBB();
    //     Operand* src1 =
...
void FuncCallExp::genCode()
{
    ...
    if(params!=NULL)//有参数就要都遍历 并且参数也要放入指令
    {
        ...
        for( std::vector<Operand*>::reverse_iterator revit=para.rbegin();revit!=para.rend();revit++)
        {
...
void Whilestmt::genCode()
{
    ...
    Instruction* inlla;
    //首先要跳转到cond语句块生成代码
    inlla=new UncondBrInstruction(Condbb); //无条件跳转到condbb
    ...
    insertbb->insertBack(inlla);//当前块要跳转到cond执行
    builder->setInsertBB(end_bb);
    
}

void Params::genCode()//函数定义的参数列表
{
    // Todo
...
void Param::genCode()//函数定义中的参数
{
    ...
    if(expr1!=NULL)
    {
        ...
        //dyt add beg
        //不在函数内部，额外在unit中添加一个存储结构，同时分配地址
        Unit *unit = builder->getUnit();
        Function *func = new Function(unit, se);
        BasicBlock *entry = func->getEntry();
        ...
        Instruction *alloca = new AllocaInstruction(addr, se);//分配地址
        entry->insertFront(alloca);
        se->setAddr(addr);
        Operand *ope=expr1->getOperand();
        //func->op(function's menber).push_back(ope);
        BasicBlock *bb=builder->getInsertBB();
        new StoreInstruction(addr, ope, bb);//存储指令
...
void Paramlist::genCode()
{
    ...
    callparamsType.push(this->dst);//放入调用队列
    if(exp2!=NULL)
    {
        exp2->genCode();
...
void DAssignStmt::genCode()//declare-assign，定义语句
{
    // Todo
    //首先是定义部分 要将id对应的表项设置类型 地址 或者是分配空间
    IdentifierSymbolEntry *se = dynamic_cast<IdentifierSymbolEntry *>(id->getSymPtr());
    //判断全局变量和局部变量
    if(se->getScope()==0)
    {
    ...
    else if(se->getScope()>=1)
    {
        ...
        addr = new Operand(addr_se);
        alloca = new AllocaInstruction(addr, se);                   
        entry->insertFront(alloca); //在头部插入分配地址指令                     
        se->setAddr(addr);
...
void Decvar::genCode()//int a;
{
    ...
    else //处理函数内部的变量
    {
        ...
        else{//处理函数参数列表 PARAM 此部分已在ID处实现
...
//ok
void FunctionDef::typeCheck()//函数未声明，及不符合重载要求的函数重复声明
{
    // Todo
    ...
    if(se->getType()->isFunc()!=1 )//不是函数
    {
        fprintf(stderr,"函数 %s 返回类型不支持",name.c_str());
    }
    
    if(params!=NULL)//函数调用时形参及实参类型或数目的不一致
    {
...
void BinaryExpr::typeCheck()
{
    ...
    if(type1 != type2)
    {
        ...
        || (type1->toStr()=="i32()" && type2->toStr()=="i32")//FunctionType
        ...
        //完全匹配情况
        ||(type1->toStr()=="i32" && type2->toStr()=="i32")
        ||(type1->toStr()=="i1" && type2->toStr()=="i1")
        ||(type1->toStr()=="i32()" && type2->toStr()=="i32()")
        ||(type1->toStr()=="constant" && type2->toStr()=="constant")
        )
        )
        {
...
{
    // Todo
    // 在parser.y中，已经对ID的定义与重定义进行过检查，这里不必重复检查
    // 482LVal左值，对未定义ID进行检查
    // 549varID，对重定义检查
...
//ok
//变量重复声明的情况：
void DeclStmt::typeCheck()//使用DeclStmt的是INT和CONST，只要考虑这两个
{
    // Todo
    Type *type1=id->getSymPtr()->getType();
    //首先判断是否为整型
    if((type1->isInt()||type1->isConst())!=1)
    {
...
//ok
void ConstDecAndAssStmt::typeCheck()//const int a = 1;
{
    // Todo
    //---------------------------------------第一部分检查定义是否类型有误
    Type *type1=id->getSymPtr()->getType();
    ...
    //首先对表达式进行类型检查
    expr->typeCheck();
    //然后判断赋值左右两边类型是否一致表达式的类型 等式右边必须是const和INT
    Type *type2=expr->getSymPtr()->getType();
