...
std::string IdentifierSymbolEntry::toStr()
{
    ...
    if(temp->isFunc()||this->getScope()==0)//全局
    {
        return "@"+name;
    }
    else
...
SymbolEntry* SymbolTable::search_inthis(std::string name)//只找当前符号表
{
    SymbolTable *cur=this;
...
SymbolEntry* SymbolTable::search_func()//获取函数符号表
{
    SymbolTable *cur=this;
